...
module BitbarCloudApiClient
  class UsersApi
    ...
    def activate_using_post_with_http_info(email, key, password, opts = {})
      ...
      if @api_client.config.client_side_validation && opts[:'hear_from'] && !['COWORKER', 'EMAIL_CAMPAIGN', 'EVENT', 'ONLINE_COMMUNITY', 'SOCIAL_NETWORK', 'WEB_SEARCH', 'OTHER'].include?(opts[:'hear_from'])
        fail ArgumentError, 'invalid value for "hear_from", must be one of COWORKER, EMAIL_CAMPAIGN, EVENT, ONLINE_COMMUNITY, SOCIAL_NETWORK, WEB_SEARCH, OTHER'
      end
      if @api_client.config.client_side_validation && opts[:'role'] && !['CEO_OR_OWNER', 'VP', 'MANAGER', 'INDIVIDUAL', 'QUALITY_ASSURANCE', 'DEVELOPER', 'MARKETING', 'CONSULTANT', 'OTHER'].include?(opts[:'role'])
        fail ArgumentError, 'invalid value for "role", must be one of CEO_OR_OWNER, VP, MANAGER, INDIVIDUAL, QUALITY_ASSURANCE, DEVELOPER, MARKETING, CONSULTANT, OTHER'
      end
      # resource path
      ...
      query_params[:'hearFrom'] = opts[:'hear_from'] if !opts[:'hear_from'].nil?
    ...
    def register_using_post1(email, opts = {})
      data, _status_code, _headers = register_using_post1_with_http_info(email, opts)
      data
    end
