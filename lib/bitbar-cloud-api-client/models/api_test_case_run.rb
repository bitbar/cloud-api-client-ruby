  ...
  class APITestCaseRun
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BitbarCloudApiClient::APITestCaseRun` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BitbarCloudApiClient::APITestCaseRun`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      result_validator = EnumAttributeValidator.new('Object', ['PASSED', 'FAILED', 'SKIPPED', 'NOT_AVAILABLE'])
    ...
    def result=(result)
      validator = EnumAttributeValidator.new('Object', ['PASSED', 'FAILED', 'SKIPPED', 'NOT_AVAILABLE'])
    ...
    def hash
      [id, self_uri, class_name, create_time, duration, error_message, method_name, result, stacktrace, steps, suite_name].hash
    end
